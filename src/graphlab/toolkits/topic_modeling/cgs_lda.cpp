...
void save_topic_assignment(const graph_type::vertex_type& vertex, uint32_t& total) {
	if(is_doc(vertex)) {
		total += 1;
		int32_t max_cnt = -1;
		int32_t max_idx = -1;
		
    for(size_t i = 0; i < vertex.data().factor.size(); ++i) {
			if(vertex.data().factor[i] >= max_cnt) {
				max_cnt = vertex.data().factor[i];
				max_idx = i;
			}
		}
		output_lock.lock();
		output_file << -(int)vertex.id() - 2 << "\t" << max_idx << "\n";
		output_lock.unlock();
	}
}
...
int main(int argc, char** argv) {
  ...
  if(!doc_dir.empty()) {
	...
	output_file.open("result.tsv");
	uint32_t saved_docs = graph.fold_vertices<uint32_t>(save_topic_assignment);
	output_file.close();
