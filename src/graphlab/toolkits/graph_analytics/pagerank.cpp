...
public:
  ...
  double gather(icontext_type& context, const vertex_type& vertex,
    ...
    return (edge.source().data() / edge.source().num_out_edges()) * edge.data();
  }
...
bool parser(graph_type& graph, const std::string& file_name, const std::string& textline)
{
	std::stringstream strm(textline);
	graphlab::vertex_id_type src(0), dest(0);
	double weight(1);

	strm >> src >> dest >> weight;
...
int main(int argc, char** argv) {
  ...
  graph_type graph(dc, clopts);
	...
	graph.load("/data/bshi/weight-net-1999-2012.txt", parser);
